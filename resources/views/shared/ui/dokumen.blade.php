...
<section class="container mx-auto px-4 grid grid-cols-1 gap-x-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
    @forelse($documents as $document)
        <div
            class="group mt-10 relative bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100">
                <form action="{{ route('dokumen.hapus_dokumen', $document->id) }}" method="POST"
            ...
            <a href="{{ route('dokumen.unduh_dokumen', $document->id) }}"
                ...
                <div class="p-6 flex flex-col items-center justify-center h-40 space-y-3">
                    @if (Str::endsWith($document->file_path, '.pdf'))
                        <span class="relative">
                            <i
                                class="fas fa-file-pdf text-5xl text-red-500/90 group-hover:text-red-600 transition-colors duration-200"></i>
                            <i
                                class="fas fa-download text-[10px] text-red-500 absolute -bottom-1 -right-1 p-1 bg-red-100 rounded-full flex items-center justify-center"></i>
                        </span>
                    @elseif(Str::endsWith($document->file_path, ['.doc', '.docx']))
                        <span class="relative">
                            <i
                                class="fas fa-file-word text-5xl text-blue-500/90 group-hover:text-blue-600 transition-colors duration-200"></i>
                            <i
                                class="fas fa-download absolute -right-1 -bottom-1 flex items-center justify-center p-1 text-[10px] text-blue-500 bg-blue-100 rounded-full"></i>
                        </span>
                    @elseif(Str::endsWith($document->file_path, ['.xls', '.xlsx']))
                        <span class="relative">
                            <i
                                class="fas fa-file-excel text-5xl text-green-500/90 group-hover:text-green-600 transition-colors duration-200"></i>
                            <i
                                class="fas fa-download text-[10px] text-green-500 absolute -bottom-1 -right-1 p-1 bg-green-100 rounded-full flex items-center justify-center"></i>
                        </span>
                    @else
                        <span class="relative">
                            <i
                                class="fas fa-file text-5xl text-gray-500/90 group-hover:text-gray-600 transition-colors duration-200"></i>
                            <i
                                class="fas fa-download text-[10px] text-gray-500 absolute -bottom-1 -right-1 p-1 bg-gray-100 rounded-full flex items-center justify-center"></i>
                        </span>
                    @endif
                </div>
    ...
    @empty
        <span
            class="col-span-full cursor-default flex flex-col items-center justify-center py-24 px-4 text-center space-y-3">
    ...
    @endforelse
        <figure
            ...
            <span
                class="absolute inset-0 bg-blue-500/0 group-hover:bg-blue-500/[0.02] rounded-xl transition-colors duration-300"></span>
            <div class="relative p-6 flex flex-col items-center justify-center h-40 space-y-3">
                <i
                    class="fas fa-cloud-upload-alt text-5xl text-blue-500/90 group-hover:text-blue-600 transition-colors duration-200"></i>
                <i
                    class="fas fa-plus text-[10px] text-blue-500 absolute top-1/2 right-3/8 p-1 bg-blue-100 rounded-full flex items-center justify-center"></i>
            </div>
