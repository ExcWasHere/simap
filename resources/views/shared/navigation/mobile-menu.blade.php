<section class="hidden flex-col z-40 fixed top-16 inset-x-0 mx-4 bg-white border border-gray-200 rounded-lg shadow-lg lg:hidden" id="mobile-menu">
    <nav class="p-2 space-y-1">
        ...
        <a href="{{ route('monitoring') }}" class="block px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-md transition-colors {{ Request::is('monitoring*') ? 'font-medium text-blue-600 bg-blue-50' : '' }}">
            Monitoring
        </a>
    </nav>
    <hr class="h-1 text-gray-200" />
    <form method="POST" action="{{ route('keluar') }}" class="p-2">
        @csrf
